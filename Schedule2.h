...
public:
    ...
    Schedule2(int n) : roomsHash(100), courses(n), coursesNum(n) {} //here we must insert dummy nodes into the roomHahs table's Lists
    ...
    void addRoom(int roomId) {
        ...
        roomsHash.insert(roomId, IntArray(10)); //shouldn't this array be not boolean, but with the course numbers??
    }

    void deleteRoom(int roomId) {
        if (roomId <= 0)
            throw InvalidInput();
        if (!roomsHash.member(roomId)) //todo: fix to not...
            throw Failure();
